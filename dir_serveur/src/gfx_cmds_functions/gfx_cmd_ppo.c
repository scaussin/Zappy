...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2017/03/03 18:47:04 by aleung-c          #+#    #+#             */
/*   Updated: 2017/03/03 18:47:07 by aleung-c         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
/*
**	gfx command ppo: asking for the position and direction of one player
**	by its sock number.
*/
...
void	gfx_cmd_ppo(t_serveur *serv, t_client_entity *gfx_client, char *param)
{
	...
	{
		printf(KMAG "Bad format to gfx cmd [ppo] from gfx client\n" KRESET);
		return ;
	}
	...
	while (client_tmp)
	{
		if (client_tmp->sock == client_nb && !client_tmp->is_gfx
			...
			return (ppo_send_msg(serv, client_tmp));
...
void	ppo_send_msg(t_serveur *serv, t_client_entity *client_tmp)
{
	char				*gfx_msg;
